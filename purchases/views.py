...
def new(request):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            History(user=request.user, content=u'New purchase#%d added.' % purchase.id).save()
...
def edit(request, id):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            History(user=request.user, content=u'Purchase#%d edited.' % purchase.id).save()
            return redirect(purchase)
...
def delete(request, id):
    ...
    History(user=request.user, content=u'Purchase#%d deleted.' % purchase.id).save()
...
def pay(request, id):
    ...
    History(user=request.user, content=u'Purchase#%d paid.' % purchase.id).save()
    return redirect(purchase)
